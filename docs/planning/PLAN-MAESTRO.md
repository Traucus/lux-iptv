...
| Vault host-only + refresh; TMDB key onboarding UI | Row hydration + start enrichment from saved key (PA-06, remainder of FA-08) |
...
| **F4** | TMDB layer | Posters, fanart, synopsis, rating on Home, rows, detail. Key is part of the product | Onboarding UI in tree. Remaining: start enrichment from saved key; row hydration (PA-06) |
