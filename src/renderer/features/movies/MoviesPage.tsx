...
export function MoviesPage(): React.ReactElement {
  ...
  const [allMovies, setAllMovies] = useState<MoviePosterData[]>([]);

  const { data: groups } = useCatalogGroups('movie');
  const { data, isLoading, error, isFetching } = useCatalogList('movie', {
    limit: PAGE_SIZE,
    offset,
    groupTitle: selectedGroup || undefined,
  });

  // Accumulate movies as user loads more
  React.useEffect(() => {
    if (data?.items) {
      if (offset === 0) {
        setAllMovies(data.items.map((item) => ({
          id: item.id,
          name: item.name,
          year: item.year,
