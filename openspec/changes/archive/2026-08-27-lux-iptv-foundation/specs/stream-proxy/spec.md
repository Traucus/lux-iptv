...
The system MUST provide a stream proxy service that intercepts outgoing stream requests using Electron's `net` module. The proxy MUST sit between the player and the origin server.

#### Scenario: Proxy intercepts stream request

- GIVEN the player requests a stream URL
- WHEN the request is routed through the proxy
- THEN the proxy MUST use Electron `net.request()` to fetch the stream

#### Scenario: Proxy forwards response body

- GIVEN the origin server returns a valid HLS segment
- WHEN the proxy receives the response
- THEN it MUST stream the body back to the player unchanged
